...
# Install plaid-python and its dependencies into the python directory
RUN pip install plaid-python -t python/ && \
...
RUN zip -r /plaid_layer.zip python/ -x "*.pyc" "*/__pycache__/*"

# Optional: Copy the zip file to a mounted volume or output location
CMD ["cp", "/plaid_layer.zip", "/output/"]